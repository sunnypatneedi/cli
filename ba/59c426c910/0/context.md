...
Including APIKey and Endpoint in EventPayload means the detached subprocess is invoked with a JSON string containing the PostHog key and all telemetry properties as a command-line argument (see spawnDetachedAnalytics), which makes this data visible via ps/process listings and potentially in logs. To reduce the risk of accidentally exposing the PostHog key and event metadata, consider removing these fields from the payload and having SendEvent read PostHogAPIKey/PostHogEndpoint directly from the ...
